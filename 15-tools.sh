...
emerge -vq -j15 --load-average 8 app-admin/sysklogd
rc-update add sysklogd default
emerge -vq -j15 --load-average 8 sys-process/cronie
rc-update add cronie default
emerge -vq -j15 --load-average 8 sys-apps/mlocate
rc-update add sshd default
emerge -vq -j15 --load-average 8 net-misc/dhcpcd
mkdir -p /etc/portage/package.use/app-editors/
echo ">=app-editors/vim-7.4.769 minimal" > /etc/portage/package.use/app-editors/vim
emerge -vq -j15 --load-average 8 app-editors/vim
emerge -vq -j15 --load-average 8 dev-vcs/git
emerge -vq -j15 --load-average 8 sys-process/lsof
emerge -vq -j15 --load-average 8 sys-process/htop
...
DATA
emerge -vq -j15 --load-average 8 app-emulation/virtualbox
...
DATA
emerge -vq -j15 --load-average 8 app-emulation/vagrant
mkdir -p /etc/portage/package.use/app-emulation/
echo ">=app-emulation/qemu-2.7.0 xen virtfs ssh" > /etc/portage/package.use/app-emulation/qemu
QEMU_SOFTMMU_TARGETS="x86_64 arm i386" emerge -vq -j15 --load-average 8 app-emulation/qemu
EOF
